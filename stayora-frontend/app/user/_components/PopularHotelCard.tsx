...
export default function PopularHotelCard({
  ...
  return (
    <div
      className="relative bg-white border border-gray-200 overflow-hidden cursor-pointer group hover:border-gray-300 transition-colors shadow-sm"
      onClick={() => {
    ...
    >
      ...
      <div className="relative h-44 overflow-hidden bg-gray-100">
        ...
        <button
          ...
          className={`absolute top-3 right-3 w-8 h-8 bg-white/80 backdrop-blur-sm border border-gray-200 flex items-center justify-center transition-all z-10 hover:border-[#059669] ${isProcessing ? "opacity-50 cursor-not-allowed" : "cursor-pointer"} rounded`}
        ...
        {/* Rating badge */}
        {rating !== undefined && (
          <div className="absolute top-3 left-3 bg-black/60 backdrop-blur-sm border border-white/20 px-2 py-1 flex items-center gap-1 rounded">
            <Star size={9} className="text-[#059669] fill-[#059669]" />
      ...
      {/* Content */}
      <div className="p-4">
        <p className="text-[#059669] text-[9px] tracking-[0.18em] uppercase mb-1.5 truncate">
          {location}
        </p>
        <h3 className="text-gray-900 text-sm font-bold uppercase mb-3 truncate leading-snug font-heading"
        >
          {name}
        </h3>
        <div className="flex items-end justify-between border-t border-gray-200 pt-3">
          <div>
            <span className="text-gray-900 text-base font-bold font-heading"
            >
              Rs. {price.toLocaleString()}
            </span>
            <span className="text-gray-500 text-[11px] ml-1">/night</span>
          </div>
          <span className="text-[#059669] text-[9px] tracking-[0.14em] uppercase opacity-0 group-hover:opacity-100 transition-opacity">
            Book →
          </span>
        </div>
      </div>
