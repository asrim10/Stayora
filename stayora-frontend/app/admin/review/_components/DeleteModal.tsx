...
const DeleteModal = ({
  ...
  >
    <div className="w-full max-w-95 rounded-xl border border-red-200 bg-white p-8 text-center shadow-2xl">
      ...
      <p
        className="mb-5 text-base font-semibold text-[#059669]"
        style={{ fontFamily: "'Cormorant Garamond', serif" }}
      >
        {review.fullName}
      </p>
      ...
      <div className="flex gap-2.5">
        <button
          onClick={onClose}
          className="flex-1 py-2.5 rounded-lg border border-gray-300 bg-white text-sm text-gray-500 hover:border-gray-400 hover:text-gray-700 transition-all cursor-pointer"
        >
          Cancel
        </button>
        <button
          onClick={onConfirm}
          disabled={loading}
          className="flex-1 py-2.5 rounded-lg border border-red-300 bg-white text-sm text-red-600 hover:bg-red-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
        >
