      ...
      <div className="grid grid-cols-2 gap-2.5">
        {ACTIONS.map((a) => (
          <a
            key={a.label}
            href={a.href}
            className="flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 hover:border-gray-300 hover:bg-gray-100 transition-all duration-200 group"
          >
            <span className="text-lg">{a.icon}</span>
            <span className="text-xs text-gray-600 group-hover:text-gray-900 transition-colors uppercase tracking-wide">
              {a.label}
            </span>
          </a>
        ))}
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <div className="mb-5">
        ...
        {checkIns.length === 0 ? (
          <p className="text-xs text-gray-500 pl-3.5">None upcoming</p>
        ) : (
          checkIns.slice(0, 3).map((b) => (
            ...
            >
              <div className="min-w-0">
                <p className="text-xs text-gray-700 truncate">{getGuest(b)}</p>
